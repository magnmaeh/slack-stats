...
def get_images(emojinames):
    ...
    for name in emojinames:
        for file in emojifiles:
            if name in file and not name in emojis_found:
                imgs.append(image.imread(emojidir + '/' + file))
                emojis_found.append(name)

    if len(emojis_found) != len(emojinames):
        set_difference = set(emojinames) - set(emojis_found)
        not_founds = list(set_difference)
        
        print("\nThe following emojis do not have any image to plot:")
        for not_found in not_founds:
            print("* " + not_found)
        print("\nYou will need to fetch them and provide them in \'emojis\'.")
        return []
    return imgs
